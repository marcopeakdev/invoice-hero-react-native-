...
export const DependFieldsInvoiceItems: React.FC<Props> = ({ isEstimate = false }) => {
  const [discountToggleType, setDiscountToggleType] = useState(true);
  const [taxToggleType, setTaxToggleType] = useState(true);

  const {
    values: { items, subTotal, discount, tax, total, deposit },
    setFieldValue,
  ...
  useEffect(() => {
    ...
    setFieldValue(
      'total',
      subTotal - (discount ? Number(discount) : 0) + (tax ? Number(tax) : 0) - (deposit ? Number(deposit) : 0),
    );
  ...
  const onDepositChanged = (val: string) => {
    if (val === '' || !subTotal) {
      setFieldValue('deposit', '');
      return;
    }

    setFieldValue('deposit', String(Number.isNaN(val) ? 0 : val));
  };
    ...
    <View style={styles.container}>
      ...
      {!isEstimate && (
        <InputField
          ...
          name={'deposit'}
          onManualChange={onDepositChanged}
          containerStyle={styles.field}
        />
      )}
